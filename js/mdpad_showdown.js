...
//
    ...
    text = _DoMdpadCalculateButton(text);
	text = _DoYamlCodeBlocks(text);
	text = _DoMdpadGithubCodeBlocks(text);
...
var _DoMdpadCalculateButton = function(text) {
//
...
//
	...
	text = text.replace(/\[\[\[Calculate\]\]\]/g,
		'<input type="button" onclick="javascript:mdpad.calculate()" value="Calculate"></input>');
...
var id = generateId("mdpad_res_", 1);

var _DoMdpadGithubCodeBlocks = function(text) {
//
...
//
	...
	text = text.replace(/(?:^|\n)```.*js(.*)\n([\s\S]*?)\n```/g,
		function(wholeMatch,m1,m2) {
			...
			codeblock = "<div class=\"jsblock\" " + language + " run='normal'><pre class='jsinput'><code>" + codeblock + 
                        "\n</code></pre><div class = \"jsresult\" id=\"" + id() + "\"></div></div>";
...
var _DoYamlCodeBlocks = function(text) {
//
//  Process Github-style code blocks
//  Example:
//  ```yaml
...
console.log(extras)
			...
			codeblock = codeblock.replace(/^\n+/g,""); // trim leading newlines
			codeblock = codeblock.replace(/\n+$/g,""); // trim trailing whitespace
			codeblock = "<div class='yamlblock' " + extras + "><pre class='yamlinput'><code>" + codeblock + 
                        "\n</code></pre><div class = \"yamlresult\" id=\"" + id() + "\"></div></div>";
			return hashBlock(codeblock);
		}
	);
